...
### Face names
A face id is a number, and a boundary condition is per vessel, so something has to say which cap is the azygous vein. Where the input surface came from [Clip vessel](ClipVessel.md) the answer is already in the scene: the names are the labels on the clip points, and Clip Vessel records on the surface where to find them - a *ClipPoints* node reference to the markups node, and the attributes `ClipVessel.FaceIdToClipPointID` and `ClipVessel.WallFaceID`.

Clip Vessel writes them on every run, including the one this module is usually handed: an uncapped surface with no face id array on it at all, whose ids this module works out for itself from the boundary labels. The record and those ids are one numbering.

This module copies all three onto the volume mesh, and onto the remeshed surface, without reading them. It is in the middle of the chain: the face ids carry through meshing, and the names have to carry with them, or a case setup downstream is back to naming twenty-odd caps by eye. SimVascular Mesh Prep, in the SlicerSimVascular extension, is what reads them at the far end.

A mesh made from a surface that was not clipped in Clip Vessel gets no record, and any record left on the output node by a previous run is cleared rather than kept - it would name this mesh's faces after another case's vessels, and plausibly, since both are only numbers. If a face named in the record has no cells on the finished mesh, the module says so: meshing can lose a cap, a boundary layer strips them and remakes them, and this is the last point at which that is cheap to notice.
