...
//---------------------------------------------------------------------------
bool vtkSlicerStenosisMeasurement3DLogic::Process(vtkMRMLMarkupsShapeNode * wallShapeNode,
                                                  ...
                                                  vtkVariantArray * results, vtkMRMLTableNode * outputTableNode)
{
  ...
  if (!lumenSegmentationNode->GetClosedSurfaceRepresentation(segmentID, inputLumenSurface))
  {
    ...
    if (!lumenSegmentationNode->GetClosedSurfaceRepresentation(segmentID, inputLumenSurface))
    {
      vtkErrorMacro("Cannot get closed surface from segmentation.");
      return false;
    }
  }

  vtkNew<vtkPolyData> inputLumenEnclosed;
  EnclosingType enclosingType = this->GetClosedSurfaceEnclosingType(wallClosedSurface, inputLumenSurface, inputLumenEnclosed);
  if (enclosingType == EnclosingType::EnclosingType_Last)
  {
    return false; // Logging has been done.
  }
  if (enclosingType == EnclosingType::Distinct)
  {
    vtkErrorMacro("Input tube and input lumen do not intersect.");
    return false;
  }

  vtkNew<vtkPolyData> lumenSurface;
  if (!this->UpdateClosedSurfaceMesh(inputLumenEnclosed, lumenSurface))
  {
    vtkWarningMacro("Error updating the clipped lumen; continuing with the raw clipped surface.");
    lumenSurface->Initialize();
    lumenSurface->DeepCopy(inputLumenEnclosed);
  }
  ...
  if (!this->ClipClosedSurface(lumenSurface, lumenIntermediate, p1, startDirection, false))
  {
    return false;
  }
  ...
  if (!this->ClipClosedSurfaceWithClosedOutput(lumenSurface, outputLumenClosedPolyData, p1, startDirection, p2, endDirection))
  {
    return false;
  }
            ...
            double * startOrigin, double * startNormal, double * endOrigin, double * endNormal)
{
  ...
  if (startNormal[0] == 0 and startNormal[1] == 0 and startNormal[2] == 0)
  {
    vtkErrorMacro("Invalid start normal, all values are zero.");
    return false;
  }
  if (endNormal[0] == 0 and endNormal[1] == 0 and endNormal[2] == 0)
  {
    vtkErrorMacro("Invalid end normal, all values are zero.");
    return false;
  }
  if ((startOrigin[0] == endOrigin[0])
    and (startOrigin[1] == endOrigin[1])
    and (startOrigin[2] == endOrigin[2]))
  {
                                                       ...
                                                       vtkPolyData * lesion)
{
  ...
  {
    vtkErrorMacro("Invalid output: 2 polydata objects must be available for open lumen/wall output.");
    return false;
  }
  // Get the spline polydata from the shape markups node.
  vtkSmartPointer<vtkPolyData> spline = vtkSmartPointer<vtkPolyData>::New();
  ...
  array->SetNumberOfValues(outputWallOpenPolyData->GetNumberOfPoints());
  array->Fill(0);
  for (int i = 0; i < outputLumenOpenPolyData->GetNumberOfPoints(); i++)
  {
    array->InsertNextValue(1);
  }
  vtkNew<vtkAppendPolyData> appender;
  appender->AddInputData(outputWallOpenPolyData);
  appender->AddInputData(outputLumenOpenPolyData);
  appender->Update();
