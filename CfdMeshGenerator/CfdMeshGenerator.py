...
prismatic boundary layer, and filled by TetGen. Every cell of the result carries the id of the
...
developed by Luca Antiga and David Steinman. The volume mesh is generated by TetGen, by Hang Si.
...
class CfdMeshGeneratorLogic(ScriptedLoadableModuleLogic):
    ...
    def __init__(self) -> None:
        ...
        # Whether the last run ended with TetGen refusing to fill the surface it was given. The
        # mesh is handed back anyway - what there is of it is the thing to look at to see why -
        ...
        if self.lastTetrahedralizationFailed:
            if parameters.boundaryLayer:
                raise RuntimeError(_(
                    "TetGen could not fill the space left inside the boundary layer, so the "
                    ...
                    "give it more substeps to settle over."))
            raise RuntimeError(_(
                "TetGen could not fill the surface with tetrahedra, so the output holds only the "
                "surface. The surface it was given is not one it can fill: check that the input "
                "is closed and does not cross itself."))
    ...
    def generateMesh(self, surface, *,
        ...
        inside with a prismatic boundary layer, and whatever space is left over is filled by
        TetGen. Every cell of the result carries an entity id in cellEntityIdsArrayName:
        ...
        # One size for the volume elements, from the size the surface was meshed at: TetGen is
        # given it as a cap rather than as a function of position (see runTetGen).
        maxElementVolume = self.maximumElementVolume(targetEdgeLength, volumeElementScaleFactor)

        if not boundaryLayer:
            mesh = self.fillWithTetrahedra(remeshedSurface, cellEntityIdsArrayName,
                                           maxElementVolume, outputSurfaceElements=True)
        else:
    ...
    def fillWithTetrahedra(self, surface, cellEntityIdsArrayName, maxElementVolume,
                           outputSurfaceElements):
        """The volume enclosed by the surface, filled with tetrahedra of at most the given
        volume (vmtktetgen)."""
        surfaceMesh = self.surfaceToMesh(surface)

        self.log(_("Generating volume mesh"))
        mesh = self.runTetGen(surfaceMesh, cellEntityIdsArrayName,
                              outputSurfaceElements=outputSurfaceElements,
                              maxElementVolume=maxElementVolume)

        if mesh.GetNumberOfCells() == 0 and surfaceMesh.GetNumberOfCells() > 0:
    ...
    def meshWithBoundaryLayer(self, remeshedSurface, referenceSurface, cellEntityIdsArrayName, *,
                              ...
                              maxElementVolume, boundaryLayerOnCaps, numberOfSubLayers,
        ...
        self.log(_("Generating volume mesh"))
        # The surface elements of the inner surface are the inner face of the boundary layer,
        # which the layer itself already carries; only the tetrahedra are wanted from here.
        volumeMesh = self.runTetGen(innerSurfaceMesh, cellEntityIdsArrayName,
                                    outputSurfaceElements=False,
                                    maxElementVolume=maxElementVolume)
        if volumeMesh.GetNumberOfCells() == 0 and outerSurfaceMesh.GetNumberOfCells() > 0:
        ...
        if not boundaryLayerOnCaps:
            # The caps were made on the inner surface, past the boundary layer, so they are taken
            # from there: everything the capper numbered above the wall.
            threshold = vtk.vtkThreshold()
            threshold.SetInputData(innerSurfaceMesh)
            threshold.SetInputArrayToProcess(0, 0, 0, vtk.vtkDataObject.FIELD_ASSOCIATION_CELLS,
                                             cellEntityIdsArrayName)
            threshold.SetThresholdFunction(vtk.vtkThreshold.THRESHOLD_UPPER)
            threshold.SetUpperThreshold(self.wallCellEntityId + 0.5)
            threshold.Update()
    ...
    @staticmethod
    def maximumElementVolume(targetEdgeLength, volumeElementScaleFactor):
...
class CfdMeshGeneratorTest(ScriptedLoadableModuleTest):
    ...
    def test_CfdMeshGenerator1(self):
        ...
        mesh, remeshedSurface = logic.generateMesh(self.openTube(), targetEdgeLength=0.4)
    ...
    def test_CfdMeshGeneratorWithoutRemeshing(self):
        ...
        mesh, remeshedSurface = logic.generateMesh(surface, skipRemeshing=True)

        self.assertEqual(set(remeshedSurface.GetCellType(cellId)
                             for cellId in range(remeshedSurface.GetNumberOfCells())),
                         {vtk.VTK_TRIANGLE}, "the surface was handed on with a polygon in it")
    ...
    def test_CfdMeshGeneratorBoundaryLayer(self):
        ...
        for onCaps in (True, False):
            mesh, _remeshedSurface = logic.generateMesh(
                ...
                boundaryLayerOnCaps=onCaps)

            cellTypes = vtk.vtkCellTypes()
            mesh.GetCellTypes(cellTypes)
            self.assertTrue(cellTypes.IsType(vtk.VTK_WEDGE),
                            "the boundary layer holds no prisms (on caps: %s)" % onCaps)
            self.assertTrue(cellTypes.IsType(vtk.VTK_TETRA),
                            "the mesh is hollow inside its boundary layer (on caps: %s)" % onCaps)
    ...
    def test_CfdMeshGeneratorLayerOffImportedCaps(self):
        ...
        for onCaps in (False, True):
            ...
            self.assertIsNotNone(gap, "the mesh holds no tetrahedra (on caps: %s)" % onCaps)
            if onCaps:
                self.assertGreater(gap, 0.5 * layerThickness,
                                   "no layer was grown over the cap")
            else:
                self.assertLess(gap, 0.5 * layerThickness,
                                "the layer was grown over the cap after all")
            self.assertEqual(self.cellEntityIds(mesh, "ModelFaceID"), {0, 1, 2, 3},
                             "the caps did not come back under the ids they arrived with")
    ...
    def test_CfdMeshGeneratorNamesCapsAfterVesselEnds(self):
        ...
        for keepLabels in (True, False):
            ...
            self.assertLess(middles[2], 1.0,
                            "face 2 came back on the wrong end (labels kept: %s)" % keepLabels)
            self.assertGreater(middles[3], 9.0,
                               "face 3 came back on the wrong end (labels kept: %s)" % keepLabels)
