...
from slicer.parameterNodeWrapper import parameterNodeWrapper, Maximum, Minimum
...
from CfdMeshGeneratorLib.MeshingPipeline import (  # noqa: F401 - re-exported for scripts and tests
    ...
    FTETWILD_REQUIREMENT, NETGEN_REQUIREMENT, MINIMUM_SUBSTEPS, UNLIMITED_EDGE_LENGTH,
...
prismatic boundary layer, and filled by TetGen, fTetWild or Netgen. Every cell of the result carries the id of the
...
Netgen is the mesh generator of Joachim Schöberl and the NGSolve project
(https://github.com/NGSolve/netgen), under the GNU Lesser General Public License 2.1. It is used
through the netgen-mesher package, which the module downloads from PyPI the first time Netgen is
asked for.
""")
...
class CfdMeshGeneratorParameterNode:
    ...
    coarsen: bool = False
    netgenGrading: Annotated[float, Minimum(0.0), Maximum(1.0)] = 0.3
    netgenOptimizationSteps: Annotated[int, Minimum(0)] = 3
...
class CfdMeshGeneratorWidget(ScriptedLoadableModuleWidget, VTKObservationMixin):
    ...
    def onReload(self) -> None:
        ...
        from CfdMeshGeneratorLib import FTetWild, MeshingPipeline, MeshingWorker, Netgen

        for module in (CfdMeshGeneratorLib, FTetWild, Netgen, MeshingPipeline, MeshingWorker):
    ...
    def _updateEnabledStates(self) -> None:
        ...
        # What only Netgen reads: how fast its elements may grow, and how long it improves them.
        usingNetgen = self._parameterNode.mesher == Mesher.NETGEN
        for widget in (self.ui.netgenGradingLabel, self.ui.netgenGradingSpinBox,
                       self.ui.netgenOptimizationStepsLabel,
                       self.ui.netgenOptimizationStepsSpinBox):
            widget.enabled = usingNetgen
    ...
    def onApplyButton(self) -> None:
        ...
        if self._parameterNode.mesher == Mesher.FTETWILD and not self.logic.isFTetWildAvailable():
            if not self.offerToInstallFTetWild():
                return
        if self._parameterNode.mesher == Mesher.NETGEN and not self.logic.isNetgenAvailable():
    ...
    def offerToInstallNetgen(self) -> bool:
        """Ask to download Netgen, and say whether it can be used afterwards. Asked for the
        same reason fTetWild is: a press of Apply is not permission to reach for the network."""
        if not slicer.util.confirmOkCancelDisplay(self.logic.describeNetgenInstallation()):
            return False
        with slicer.util.tryWithErrorDisplay(_("Failed to install Netgen."), waitCursor=True):
            self.logic.installNetgen()
        return self.logic.isNetgenAvailable()
...
class CfdMeshGeneratorLogic(ScriptedLoadableModuleLogic, MeshingPipeline):
    ...
    #
    # How Netgen gets here. Always into this Python: its package has a wheel for every Python
    # Slicer runs on, so none of the environment-making fTetWild needs on some Macs applies.
    #

    @staticmethod
    def describeNetgenInstallation():
        """What installNetgen() would do, for a dialog to ask about."""
        return _(
            "Netgen is not installed. It comes as the Python package {requirement}, under the "
            "GNU Lesser General Public License 2.1, which can be downloaded and installed now. "
            "This needs an internet connection and takes a moment; it is only done once."
        ).format(requirement=NETGEN_REQUIREMENT)

    def installNetgen(self):
        """Install Netgen into this Python. Reaches out to the network."""
        slicer.util.pip_install(NETGEN_REQUIREMENT)
        if not self.isNetgenAvailable():
            raise RuntimeError(_("Netgen was installed, but still cannot be run."))
    ...
    def meshingArguments(cls, parameters: CfdMeshGeneratorParameterNode) -> dict:
        ...
        return dict(
            ...
            netgenGrading=parameters.netgenGrading,
            netgenOptimizationSteps=parameters.netgenOptimizationSteps,
...
class CfdMeshGeneratorTest(ScriptedLoadableModuleTest):
    ...
    def test_CfdMeshGenerator1(self):
        ...
        TetGen only. fTetWild and Netgen are downloads, and a smoke test is not the place to
        reach for the network; Testing/Python puts every behaviour to all three.
        """
